...
// The installed Webview can need a few event-loop turns after the focus command;
// keep the wait bounded while allowing normal CI startup variance.
...
async function findFiles(directory, fileName, depth = 0, state = { entries: 0 }) {
  ...
  for (const entry of entries) {
    state.entries += 1;
    if (state.entries > maxSmokeLogSearchEntries) {
      throw new Error("The installed-extension smoke log search exceeded its bounded entry limit.");
    }
    const entryPath = join(directory, entry.name);
    if (entry.isDirectory()) {
      matches.push(...(await findFiles(entryPath, fileName, depth + 1, state)));
