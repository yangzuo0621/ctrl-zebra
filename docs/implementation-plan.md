...
本文档是 roadmap 的权威热索引，只维护任务顺序、活跃/待开始状态、进度摘要、当前/下一执行点和活动规格链接。完成任务的历史证据由 Git、合并 PR 和变更日志保留，不在仓库中维护第二套归档台账。本文档不是执行日志：不得在此持久化任务特定的实现摘要、复用/相似性审计、Build-vs-Buy、测试/CI 细节、Reviewer findings、symbol inventories 或 design rationale。普通任务执行证据保留在会话、handoff 或 PR 中；只有持久的架构或审计结论才进入所属领域文档、ADR 或例外 maintenance 记录。
...
4. 已完成阶段的历史证据仅在回归调查、设计追溯或正式修订时通过 Git、合并 PR 或变更日志查阅。
...
| 已完成任务状态、完成 PR 和完成日期 | 本文档的进度摘要、Git、合并 PR 和变更日志 |
...
本节构成当前任务状态的唯一台账，维护活跃、待开始和已完成任务的进度摘要。任务正文不重复维护状态，避免同一任务出现两个不同结论。
...
4. 任务通过全部验证后，在最终 PR 中将该任务标记为 `已完成`并填写完成 PR 和完成日期；PR
   未合入前，主干中的任务仍不算完成。
...
- 下一任务：无
- 最后更新：2026-09-03
- 历史证据：Git、合并 PR 和变更日志（常规执行无需读取历史副本）
...
- 阶段 0–23 均已完成；当前无执行任务。
- 当前没有已批准的下一阶段或下一任务；新的候选必须先经过独立的变更控制和验收定义。

## 5. 阶段规格索引

阶段 0–23 已完成；已完成阶段规格不在仓库中建立归档副本。新的活动或计划阶段规格仍放在
`docs/roadmap/phases/`，完成后随任务收尾删除，历史证据由 Git、合并 PR 和变更日志保留。

## 6. 后续能力候选顺序

阶段 0–23 已完成，当前没有已批准的 Phase 24 或其他活动任务。以下是完成 Phase 23 后的候选评估顺序，
不是已承诺的路线图任务，也不改变当前状态台账。候选项不构成授权，进入实施前必须重新完成变更控制：
...
8. MCP 远程传输与授权扩展：Streamable HTTP + OAuth；进入实施前需要独立的安全、协议和产品变更控制。
9. 更多专用模型供应商，继续验证 Provider 边界。
10. 多 Agent；只有评测证明收益后才可规划，首个版本只允许只读子 Agent。

独立维护评估另行遵循[工程机会台账](engineering-opportunities.md)：EO-012（MCP SDK-native negotiation）
是当前优先级最高的协议边界调查候选，应先完成证据和 Build-vs-Buy 决策；EO-013（Webview chat-store
responsibility density）继续暂缓。`EO-*` 不是任务编号，不改变本索引的执行顺序或状态；候选项只有经明确
批准并晋升为路线图任务或独立 maintenance 后才能实施。
