...
阶段 0–13 已完成，完整任务正文均已按阶段归档。阶段 14 已规划，等待开始。
...
| 14 | 已规划 | [阶段 14：受控的 MCP Client 与三类 Server 原语](roadmap/phases/phase-14.md) |

## 6. 后续能力候选顺序

阶段 14 已选择受控的 MCP Client 与三类 Server 原语。完成该阶段后，建议按以下顺序评估后续能力：

1. MCP 远程传输与授权扩展：Streamable HTTP + OAuth；详细记录见
   [阶段 14 后续扩展](roadmap/phases/phase-14.md#7-后续扩展记录)。
2. 更多专用模型供应商，继续验证 Provider 边界。
3. Plan/Act 模式，验证 Tool Policy 可配置性。
4. 项目级规则文件。
5. Git 状态感知和提交辅助。
6. 代码语义索引。
7. 多 Agent。
...
[#146](https://github.com/yangzuo0621/ctrl-zebra/pull/146) 交付；T1406 正在进行中，下一任务为 T1407。
