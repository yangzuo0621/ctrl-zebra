...
- 优先展示面向用户的操作动作摘要、格式化的资源 URI 及有效时间限制；原始 JSON 参数默认作为二层渐进披露内容，避免遮挡主要决策信息。
...
- 待审批卡片就近嵌入对应 Tool 卡内（Inline Approval Fusion）；成功的只读操作默认保持紧凑，运行中、失败和待审批状态获得更高视觉优先级。
- Tool 详情按“动作摘要 → 资源/影响范围 → 风险与有效时间 → 原始参数与输出”渐进披露。原始 JSON 参数与长输出默认隐藏于二级展开项中。
