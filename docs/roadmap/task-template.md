...
- 计划新增的行为与符号：
- 审计级别：TARGETED / FULL（Executor；Reviewer 可升级为 ESCALATED FULL）
- 触发 FULL audit 的条件：无 / 列出
- 定向搜索范围、命令、关键词与 engineering-opportunity 记录：
...
- 实现摘要：
- 测试结果：
- Similarity Audit：按 `docs/development.md` 记录层级相符的证据与差异
- 实际直接复用或深化的已有功能：
- 删除或替代的旧实现：无 / 列出
- 设计偏差：无 / 说明
- 完成 PR：
