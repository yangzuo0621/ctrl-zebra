...
- 三个已支持 Provider 均提供可发现的密钥保存与最小模型配置入口；任何模型列表网络请求都由
  用户操作触发，不携带工作区或会话内容，并允许退回手工配置。
...
- 用户可以重新生成、编辑重发、显式引用工作区文件、删除会话、清空历史并配置本地保留策略；
  这些操作不复用旧批准、不重放旧副作用、不把工作区引用提升为可信指令。
- Preview/GA 工程范围包括仓库治理、受审查依赖更新、旧会话迁移或只读降级、单次 Run 成本护栏、
  用户主动脱敏诊断导出、可重复发布流水线和 Marketplace 证据；实际发布仍需单独明确授权。
