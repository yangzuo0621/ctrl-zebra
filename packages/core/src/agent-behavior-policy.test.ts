...
describe("agent behavior policy", () => {
  it.each(["hello", "Hi there!", "你好！", "thank you", "who are you?"])(
    "withholds workspace tools for the simple conversation %j",
    (content) => {
      expect(shouldOfferWorkspaceTools(content)).toBe(false);
    },
  );

  it.each(["hello, list files", "read README.md", "run the tests", "你好，请检查 package.json"])(
    "keeps workspace tools available for the task %j",
    (content) => {
